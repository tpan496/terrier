...
class ReplicationLogProvider : public AbstractLogProvider {
 public:
  ReplicationLogProvider() : replication_active_(true) {}
  ...
  void HandBufferToReplication(std::unique_ptr<network::ReadBuffer> buffer) {
    std::unique_lock<std::mutex> lock(replication_latch_);
    arrived_buffer_queue_.emplace(buffer);
    replication_cv_.notify_one();
  }
 ...
 private:
  ...
  std::chrono::minutes replication_timeout_{20};
  ...
  // TODO(Gus): For now, these buffers will most likely be allocated on demand by the network layer. We could consider having a buffer pool if allocation becomes a bottleneck
  ...
  /**
   * @return true if log file contains more records, false otherwise. Can be blocking if waiting for message from master, or master has timed out
   */
  bool HasMoreRecords() override {
    ...
    //  3. A new packet has arrived
    auto timeout = replication_cv_.wait_for(lock, replication_timeout_,
        [&] { return !replication_active_ || !arrived_buffer_queue_.empty(); });
    return !timeout && replication_active_ && !arrived_buffer_queue_.empty();
  }
  ...
  bool Read(void *dest, uint32_t size) override {
    if (curr_buffer_ == nullptr || !curr_buffer_->HasMore()) {
      ...
      if (!timeout || !replication_active_) return false;

      TERRIER_ASSERT(!arrived_buffer_queue_.empty(), "If we did not shut down or timeout, CV should only wake up when a new buffer arrives");
